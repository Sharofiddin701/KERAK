...
1.
-- Eng kop ishlatilgan yilni chiqaradigan 
CREATE OR REPLACE FUNCTION most_created_year()
RETURNS INT AS $$
DECLARE
    max_year INT;
BEGIN
    SELECT EXTRACT(YEAR FROM created_ts) AS year_created
    INTO max_year
    FROM users
...
2.
-- Misol uchun 4 ni kiritsak ,4-oyda tugilganlarni barchasini chiqaradi
CREATE OR REPLACE FUNCTION month(month_input integer)
RETURNS TABLE (user_id integer,user_dob DATE) 
AS $$
BEGIN 

RETURN QUERY
SELECT users.user_id,users.user_dob
FROM users
WHERE  EXTRACT(MONTH FROM users.user_dob)=month_input;

END;
$$ LANGUAGE plpgsql;

3.
-- Yilni kiritiladi osha yilda kirganlarni chiqarib beradi. 
CREATE OR REPLACE FUNCTION get_users_by_year(year_input numeric)
RETURNS TABLE (user_first_name VARCHAR(100))
AS $$
BEGIN 
    RETURN QUERY
    SELECT users.user_first_name
    FROM users
    WHERE EXTRACT(YEAR FROM users.created_ts) = year_input;
END;
$$ LANGUAGE plpgsql;

4.
-- Jinsiga qarab nechta borligini chiqaradi nomi bilan .
create or replace function genderof(gender VARCHAR)
returns table (user_first_name VARCHAR)
as $$
begin
return QUERY
select users.user_first_name as name from users where users.user_gender=gender;
end;
$$
language plpgsql;
